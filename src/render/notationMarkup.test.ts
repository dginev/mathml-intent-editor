...
const make = (over: Partial<Concept>): Concept => ({ slug: 's', notations: [], links: [], alias: [], ...over });

describe('notationMarkup', () => {
  it('re-renders the rich MathML from tex when present (cosmetic classes restored)', () => {
    const c = make({ slug: 'power', notations: [{ tex: 'x^2', mathml: '<math><msup><mi>x</mi><mn>2</mn></msup></math>' }] });
  ...
  it('uses the stored mathml directly when there is no tex', () => {
    ...
    const c = make({ slug: 'ab', notations: [{ mathml: stored }] });
  ...
  it('uses the stored mathml until the engine has loaded', () => {
    ...
    const c = make({ slug: 'power-not-loaded', notations: [{ tex: 'x^2', mathml: stored }] });
  ...
  it('falls back to the stored mathml when the tex fails to render', () => {
    ...
    const c = make({ slug: 'broken', notations: [{ tex: '\\frac{1}', mathml: stored }] });
    expect(notationMarkup(c, engine)).toBe(stored);
  });
});
